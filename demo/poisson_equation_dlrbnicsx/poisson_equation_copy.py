...
gdim = 2
mesh, subdomains, boundaries = dolfinx.io.gmshio.read_from_msh("mesh_data/domain_poisson.msh", mpi4py.MPI.COMM_WORLD, 0, gdim=gdim)
...
generate_ann_output_set(problem, reduced_problem, Nmax, input_training_set_filepath, output_training_set_filepath, mode="Training")
...
reduced_problem.output_range[0], reduced_problem.output_range[1] = np.min(np.load(output_training_set_filepath)), np.max(np.load(output_training_set_filepath)) # NOTE Updating output_range based on the computed values instead of user guess.

print("\n")

# Validation dataset
generate_ann_input_set(input_validation_set_filepath, samples=[7,7])
generate_ann_output_set(problem, reduced_problem, Nmax, input_validation_set_filepath, output_validation_set_filepath, mode="Validation")
customDataset = CustomDataset(problem, reduced_problem, Nmax, input_validation_set_filepath, output_validation_set_filepath)
