...
print(np.max(abs(training_set_solutions_recv[training_set_indices,:] - training_set_solutions[training_set_indices,:])))
...
eigenvalues, modes, _ = rbnicsx.backends.proper_orthogonal_decomposition(snapshots_matrix, reduced_problem.inner_product_action, N=Nmax, tol=0)
...
output_training_data = generate_ann_output_set(problem, reduced_problem, Nmax, input_training_data, indices, mode="Training")
...
reduced_problem.output_range[0], reduced_problem.output_range[1] = np.min(output_training_data_recv), np.max(output_training_data_recv) # NOTE Updating output_range based on the computed values instead of user guess.

print("\n")

customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax, input_training_set_filepath, output_training_set_filepath)
...
output_validation_data = generate_ann_output_set(problem, reduced_problem, Nmax, input_validation_data, indices, mode="Validation")
...
customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax, input_validation_set_filepath, output_validation_set_filepath)
valid_dataloader = DataLoader(customDataset, batch_size=20, shuffle=False)

model = HiddenLayersNet(training_set.shape[1], [4], Nmax, Tanh()) #TODO replace Nmax with with something like reduced_problem.N
...
for epochs in range(max_epochs):
    ...
    if epochs > 0 and current_validation_loss > 1.01 * min_validation_loss and reduced_problem.regularisation == "EarlyStopping": # 1% safety margin against min_validation_loss before invoking eraly stopping criteria
...
for i in indices:
    ...
    error_numpy[i] = error_analysis(reduced_problem, problem, error_analysis_set[i,:], model, Nmax, online_nn, device=None)
...
if rank == 0:
    ...
    rb_solution = reduced_problem.reconstruct_solution(online_nn(reduced_problem, problem, online_mu, model, Nmax, device=None))
