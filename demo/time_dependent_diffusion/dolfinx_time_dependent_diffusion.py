...
uD_prev.interpolate(lambda x: 5 * np.sin(x[0]) * np.sin(x[1]))
D = ufl.as_tensor([[1., 0., 0.],[0., 1.2, 0.],[0., 0., 0.]])
...
for i in range(num_steps):
    t += dt
    print(f"Time: {t}s")
    # TODO Why is setting to zero necessary i.e. why not just overwrite
