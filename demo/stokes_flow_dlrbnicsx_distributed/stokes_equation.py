...
gdim = 2
mesh, subdomains, boundaries = dolfinx.io.gmshio.read_from_msh("mesh_data/domain_geometry.msh", mpi4py.MPI.COMM_SELF, 0, gdim=gdim)
...
class ProblemOnDeformedDomain(ProblemBase):
    ...
    def __init__(self) -> None:
        ...
        f = dolfinx.fem.Function(V) # NOTE f_term set to 0 for symmetric solution else there should be force due to gravity
...
def generate_ann_output_set_p(problem, reduced_problem, N, input_set, indices, mode=None):
    ...
    for i in indices:
        if mode == None:
            ...
            print(f"{mode} parameter number {i+1} of {input_set.shape[0]}: {input_set[i,:]}")
        output_set[i,:] = reduced_problem.project_snapshot_p(problem.solve(input_set[i,:])[1],Nmax_p).array.astype("f")
    return output_set
...
print(np.max(abs(training_set_solutions_u_recv[training_set_indices,:] - training_set_solutions_u[training_set_indices,:])))
print(np.max(abs(training_set_solutions_p_recv[training_set_indices,:] - training_set_solutions_p[training_set_indices,:])))
...
eigenvalues_u, modes_u, _ = rbnicsx.backends.proper_orthogonal_decomposition(snapshots_matrix_u, reduced_problem._inner_product_action_u, N=Nmax_u, tol=1e-12)
reduced_problem._basis_functions_u.extend(modes_u)
print(rbnicsx.io.TextLine(f"Perform POD of pressure snapshots", fill="#"))
eigenvalues_p, modes_p, _ = rbnicsx.backends.proper_orthogonal_decomposition(snapshots_matrix_p, reduced_problem._inner_product_action_p, N=Nmax_p, tol=1e-12)
...
u_output_training_data = generate_ann_output_set_u(problem, reduced_problem, Nmax_u, u_input_training_data, indices, mode="Training")
...
u_customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax_u, u_input_training_set_filepath, u_output_training_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))])
...
u_output_validation_data = generate_ann_output_set_u(problem, reduced_problem, Nmax_u, u_input_validation_data, indices, mode="Validation")
...
u_customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax_u, u_input_validation_set_filepath, u_output_validation_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1,1], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))])
...
u_model = HiddenLayersNet(training_set.shape[1], [25, 28], Nmax_u, Tanh()) #TODO replace Nmax with with something like reduced_problem.N
...
for epochs in range(u_max_epochs):
    ...
    current_training_loss = train_nn(reduced_problem, u_train_dataloader, u_model, learning_rate=1e-3, loss_func="MSE", optimizer="Adam")
    ...
    if epochs > 0 and current_validation_loss > 1.01 * u_min_validation_loss and reduced_problem.regularisation_u == "EarlyStopping": # 1% safety margin against min_validation_loss before invoking eraly stopping criteria
...
for i in indices:
    ...
    u_error_numpy[i] = error_analysis(reduced_problem, problem, u_error_analysis_set[i,:], u_model, Nmax_u, online_nn, device=None, norm_error=reduced_problem.norm_error_u, reconstruct_solution=reduced_problem.reconstruct_solution_u , input_scaling_range=np.array([-1.,1.]), output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))], index=0)
...
p_output_training_data = generate_ann_output_set_p(problem, reduced_problem, Nmax_p, p_input_training_data, indices, mode="Training")
...
p_customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax_p, p_input_training_set_filepath, p_output_training_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))])
...
p_output_validation_data = generate_ann_output_set_p(problem, reduced_problem, Nmax_p, p_input_validation_data, indices, mode="Validation")
...
p_customDataset = CustomPartitionedDataset(problem, reduced_problem, Nmax_p, p_input_validation_set_filepath, p_output_validation_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))])
# NOTE VVIP: output_range is from training and NOT from validation to ensure same scaling factors in training as well as validation
p_valid_dataloader = DataLoader(p_customDataset, batch_size=10, shuffle=False)

p_model = HiddenLayersNet(training_set.shape[1], [25, 28], Nmax_p, Tanh()) #TODO replace Nmax with with something like reduced_problem.N
...
for epochs in range(p_max_epochs):
    ...
    current_training_loss = train_nn(reduced_problem, p_train_dataloader, p_model, learning_rate=1e-3, loss_func="MSE", optimizer="Adam")
    ...
    if epochs > 0 and current_validation_loss > 1.01 * p_min_validation_loss and reduced_problem.regularisation_p == "EarlyStopping": # 1% safety margin against min_validation_loss before invoking eraly stopping criteria
        print(f"Early stopping criteria invoked at epoch: {epochs+1}")
        break
...
for i in indices:
    ...
    p_error_numpy[i] = error_analysis(reduced_problem, problem, p_error_analysis_set[i,:], p_model, Nmax_p, online_nn, device=None, norm_error=reduced_problem.norm_error_p, reconstruct_solution=reduced_problem.reconstruct_solution_p, input_scaling_range=np.array([-1.,1.]), output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))], index=1)
...
if rank == 0:
    ...
    rb_solution_u = reduced_problem.reconstruct_solution_u(online_nn(reduced_problem, problem, online_mu, u_model, Nmax_u, device=None, input_scaling_range=np.array([-1.,1.]), output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))]))
    rb_solution_p = reduced_problem.reconstruct_solution_p(online_nn(reduced_problem, problem, online_mu, p_model, Nmax_p, device=None, input_scaling_range=np.array([-1.,1.]), output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))]))
