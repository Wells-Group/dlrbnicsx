...
gmsh.model.occ.addCurveLoop([-8, 10, 11, 12], 3)
...
for i in range(1, len(edges)+1):
    gmsh.model.addPhysicalGroup(gdim, [edges[i-1][1]], edges[i-1][1])
...
with dolfinx.io.XDMFFile(MPI.COMM_WORLD, "mesh.xdmf", "w") as mesh_file_xdmf:
    ...
    mesh_file_xdmf.write_meshtags(cell_tags, mesh.occmetry)
    mesh_file_xdmf.write_meshtags(facet_tags, mesh.occmetry)
