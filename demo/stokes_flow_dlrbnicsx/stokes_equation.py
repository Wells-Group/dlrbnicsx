...
gdim = 2
mesh, subdomains, boundaries = dolfinx.io.gmshio.read_from_msh("mesh_data/domain_geometry.msh", mpi4py.MPI.COMM_WORLD, 0, gdim=gdim)
...
eigenvalues_u, modes_u, _ = rbnicsx.backends.proper_orthogonal_decomposition(snapshots_matrix_u, reduced_problem._inner_product_action_u, N=Nmax_u, tol=0)
reduced_problem._basis_functions_u.extend(modes_u)
print(rbnicsx.io.TextLine("Perform POD of pressure snapshots", fill="#"))
eigenvalues_p, modes_p, _ = rbnicsx.backends.proper_orthogonal_decomposition(snapshots_matrix_p, reduced_problem._inner_product_action_p, N=Nmax_p, tol=0)
...
generate_ann_output_set_u(problem, reduced_problem, Nmax_u, u_input_training_set_filepath, u_output_training_set_filepath, mode="Training")
u_customDataset = CustomDataset(problem, reduced_problem, Nmax_u, u_input_training_set_filepath, u_output_training_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))])
...
generate_ann_output_set_u(problem, reduced_problem, Nmax_u, u_input_validation_set_filepath, u_output_validation_set_filepath, mode="Validation")
u_customDataset = CustomDataset(problem, reduced_problem, Nmax_u, u_input_validation_set_filepath, u_output_validation_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1,1], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(u_output_training_set_filepath)), np.max(np.load(u_output_training_set_filepath))])
...
generate_ann_output_set_p(problem, reduced_problem, Nmax_p, p_input_training_set_filepath, p_output_training_set_filepath, mode="Training")
p_customDataset = CustomDataset(problem, reduced_problem, Nmax_p, p_input_training_set_filepath, p_output_training_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))])
...
generate_ann_output_set_p(problem, reduced_problem, Nmax_p, p_input_validation_set_filepath, p_output_validation_set_filepath, mode="Validation")
p_customDataset = CustomDataset(problem, reduced_problem, Nmax_p, p_input_validation_set_filepath, p_output_validation_set_filepath, input_scaling_range=[-1.,1.], output_scaling_range=[-1.,1.], input_range=np.array([[0.5,0.5],[1.,1.]]), output_range=[np.min(np.load(p_output_training_set_filepath)), np.max(np.load(p_output_training_set_filepath))])
# NOTE VVIP: output_range is from training and NOT from validation to ensure same scaling factors in training as well as validation
