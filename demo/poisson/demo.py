...
gdim = 2
mesh, subdomains, boundaries = dolfinx.io.gmshio.read_from_msh("domain.msh", MPI.COMM_WORLD, 0, gdim=gdim)
...
class PODANNReducedProblem(abc.ABC):
    ...
    def inner_product_form(self) -> ufl.Form:  # type: ignore[no-any-unimported]
        """Return the bilinear form that defines the inner product
        associated to this reduced problem.
...
def generate_training_set() -> np.typing.NDArray[np.float64]:
    """Generate an equispaced training set."""
    training_set_0 = np.linspace(0.5, 1., 6)  # arguments: min_parameter, max_parameter, number of samples
...
def generate_ann_input_set(filepath, samples=[4, 4]):
    """Generate an equispaced training set."""
    training_set_0 = np.linspace(0.5, 1., samples[0])  # arguments: min_parameter, max_parameter, number of samples
...
for epochs in range(max_epochs):
    ...
    # 1% safety margin against min_validation_loss before invoking early
    # stopping criteria
