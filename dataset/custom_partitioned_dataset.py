...
class CustomPartitionedDataset(CustomDataset):
    def __init__(self, problem, reduced_problem, N, input_file_path, output_file_path, input_scaling_range=None, output_scaling_range=None, input_range=None, output_range=None, sizes=None):
        super().__init__(problem, reduced_problem, N, input_file_path, output_file_path, input_scaling_range, output_scaling_range, input_range, output_range)
        if sizes == None:
            world_size = dist.get_world_size()
            sizes = [int(np.rint(1/world_size*len(np.load(input_file_path, allow_pickle=True)))) for _ in range(world_size-1)]
            sizes.append(len(np.load(input_file_path, allow_pickle=True))-sum(sizes))
        assert sum(sizes) == len(np.load(input_file_path, allow_pickle=True)), "Make sure that the sum of the data in distributed dataset = total number of input set"
        self.sizes = sizes
        rank = dist.get_rank()
    ...
    def __getitem__(self,idx):
        ...
        label = np.load(self.output_file_path)[self.local_indices[idx],:]#self.reduced_problem.project_snapshot(self.problem.solve(input_data),self.N).array.astype("f")
...
if __name__ == "__main__":
    ...
    custom_partitioned_dataset = CustomPartitionedDataset(problem, reduced_problem, 10, "input_data.npy", "input_data.npy")
    dataloader = torch.utils.data.DataLoader(custom_partitioned_dataset, batch_size=1, shuffle=False)
    for batch, (X, y) in enumerate(dataloader):
        print(f"Rank: {rank}")
        print(custom_partitioned_dataset.transform(np.load("input_data.npy")[custom_partitioned_dataset.local_indices[0],:]))
        print(X)
        exit()
